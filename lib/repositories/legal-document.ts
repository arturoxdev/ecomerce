...
export function findBySlugAndLocale(slug: LegalPageSlug, locale: Locale) {
  return db.query.legalPageDocuments.findFirst({
    where: and(eq(legalPageDocuments.slug, slug), eq(legalPageDocuments.locale, locale)),
  });
}
...
export function upsert(
  slug: LegalPageSlug,
  locale: Locale,
  data: Omit<typeof legalPageDocuments.$inferInsert, "slug" | "locale">,
) {
  return db
    .insert(legalPageDocuments)
    .values({ slug, locale, ...data })
    .onConflictDoUpdate({
